...
It is possible to enable deployment via a different deterministic deployment proxy (https://github.com/gnosis/safe-singleton-factory). To enable this the `CUSTOM_DETERMINISTIC_DEPLOYMENT` env var has to be set to `true` (see `.env.sample`). To make sure that the latest version of this package is install, make sure to run ` yarn add --dev @safe-fndn/safe-singleton-factory@https://github.com/safe-fndn/safe-singleton-factory.git#main` before deployment.
